...
function TooltipPersonalizado({ active, payload, label }) {
  ...
  if (value == null) return (
    <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-2 shadow text-sm">
      <p className="font-medium text-gray-700 dark:text-slate-200">{label}</p>
      <p className="text-gray-400 dark:text-slate-500 text-xs mt-1">Sin datos para este período</p>
    </div>
  )

  return (
    <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-2 shadow text-sm max-w-[220px]">
      <p className="text-xs text-gray-400 dark:text-slate-500 mb-1">{windowLabel}</p>
  ...
  return (
    <section className="px-4 py-6 sm:px-6">
      ...
      <div className="flex gap-3 mb-4 flex-wrap">
        <div>
          <label className="text-xs text-gray-500 dark:text-slate-400 block mb-1">Tu AFP</label>
        ...
        <div>
          ...
          <select className={selectCls} value={fondo} onChange={e => setFondo(e.target.value)}>
      ...
      {limitedYear && (
        <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-lg px-3 py-2 mb-3 text-xs text-amber-800 dark:text-amber-300">
          Esta AFP partió en {limitedYear}, por eso no aparecen datos anteriores a ese año.
        </div>
      )}

      {!dataset ? (
        <div className="bg-gray-50 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-lg px-4 py-6 text-center text-sm text-gray-400 dark:text-slate-400">
      ...
      <div className="flex gap-4 mt-2 text-xs text-gray-500 dark:text-slate-400 flex-wrap">
        ...
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded-sm bg-gray-200 dark:bg-slate-600 inline-block" /> Sin datos
        </span>
        {sistemaData && (
          <span className="flex items-center gap-1">
            <span className="inline-block w-5 border-t-2 border-dashed border-gray-400 dark:border-slate-500" />
            Promedio sistema
          </span>
        )}
      </div>

      <div className="mt-4 bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 rounded-lg px-4 py-3 text-sm text-blue-900 dark:text-blue-200">
